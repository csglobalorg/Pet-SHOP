...
export const CuratedSections: React.FC<CuratedSectionsProps> = ({ onSelectCategory }) => {
  ...
  return (
    <div className="space-y-6 sm:space-y-10">
      ...
      <section className="bg-white rounded-2xl p-3.5 sm:p-7 border border-slate-200/80 shadow-xs space-y-4 sm:space-y-6">
        
        {/* Header with Title and Modern Tab Switcher */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 sm:gap-4 border-b border-slate-100 pb-3 sm:pb-4">
          <div>
          ...
          <div className="flex items-center gap-1.5 bg-slate-100 p-1 rounded-xl w-full sm:w-auto overflow-x-auto no-scrollbar">
            <button
              ...
              className={`flex-1 sm:flex-initial text-center whitespace-nowrap px-3 sm:px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all cursor-pointer inline-flex items-center justify-center gap-1.5 ${
                activeTab === 'cat'
      ...
      <section className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-900 via-slate-900 to-purple-950 text-white p-4 sm:p-8 shadow-sm border border-slate-800">
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-5 sm:gap-6">
          ...
          <div className="space-y-2 text-center md:text-left max-w-xl">
            ...
            <div className="flex flex-wrap items-center gap-3 sm:gap-4 pt-2 text-xs text-slate-300 justify-center md:justify-start">
          ...
          <div className="w-full max-w-xs sm:w-auto shrink-0 flex justify-center">
            <div className="w-full sm:w-64 h-36 sm:h-38 rounded-2xl bg-gradient-to-tr from-purple-900/90 via-slate-800 to-slate-900 p-4 border border-purple-400/20 shadow-xl flex flex-col justify-between">
      ...
      <section id="brands-section" className="space-y-3">
        <div className="flex items-center justify-between border-b border-slate-100 pb-2">
          <div>
            <h2 className="text-sm sm:text-lg font-bold text-slate-900">Authentic Brand Partners</h2>
            <p className="text-[11px] sm:text-xs text-slate-500">100% genuine imported pet nutrition and accessories</p>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-8 gap-2 sm:gap-2.5">
          {POPULAR_BRANDS.map((brand, idx) => (
            <div
              key={idx}
              className="p-2.5 sm:p-3 bg-white rounded-xl border border-slate-200/80 hover:border-purple-300 hover:shadow-xs transition-all flex flex-col items-center justify-center text-center gap-0.5 group"
            >
